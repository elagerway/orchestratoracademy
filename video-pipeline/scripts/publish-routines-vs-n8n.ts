...
const forumTitle = "Claude Routines vs n8n vs Make — when to reach for which";
const forumBody = `Anthropic shipped **Claude Routines** — saved Claude Code configurations that run on a schedule, on API call, or on GitHub events. It is Anthropic infrastructure running the job, not your laptop.
...
Full breakdown with the decision tree: [Claude Routines vs n8n vs Make.com](/blog/claude-routines-vs-n8n-vs-make).`;
...
async function run() {
  ...
  const { data: f, error: fe } = await sb.from("forum_posts").insert({
  ...
  if (fe) throw fe;
  console.log("forum:", "https://www.orchestratoracademy.com/dashboard/support?post=" + f.id);
  ...
  if (be) throw be;
  console.log("blog:", "https://www.orchestratoracademy.com/blog/" + b.slug);
}
