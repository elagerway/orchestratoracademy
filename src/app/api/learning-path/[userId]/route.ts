...
export async function GET(request: Request, { params }: LearningPathProps) {
  ...
  // Users can only view their own learning path
  if (user.id !== userId) {
  ...
  // Fetch completed lesson counts per course
  ...
  if (maturityLevel <= 2) {
    // Beginner: start with foundations, then basics
    recommended = (courses ?? [])
      .filter((c) => c.is_free || c.order <= 3)
      .map((c) => c.slug);
  } else if (maturityLevel <= 3) {
    // Intermediate: skip basics, focus on applied courses
    recommended = (courses ?? [])
      .filter((c) => c.order >= 2)
      .map((c) => c.slug);
  } else {
    // Advanced: focus on advanced + security + self-improving
    recommended = (courses ?? [])
      .filter((c) => c.order >= 4)
      .map((c) => c.slug);
  }
